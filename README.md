...
### 2. 날씨 기반 일기 작성
- 사용자가 일기를 작성하면 시스템이 자동으로 해당 날짜의 날씨를 조회합니다.
- 날씨 데이터와 일기 내용을 함께 저장합니다.
- 날짜별 일기 조회 가능
- 저장된 일기 수정 및 삭제 기능 제공
...
| IDE         | IntelliJ IDEA                 |
| Weather API | Naver Weather Crawling (Optional), 기상청 오픈API, OpenWeatherMap API |

---

## 📚 API 문서화 도구

- Swagger (OpenAPI UI) — 실시간 API 테스트 기능 제공  
- ReDoc — Swagger 문서를 기반으로 한 깔끔한 문서 렌더링  
- GitBook — 프로젝트 기술 문서 및 설명서 작성  
...
├─ 📂 domain           # 엔티티 클래스
├─ 📂 dto              # 데이터 전달 객체
├─ 📂 config           # 설정 파일 (Swagger, API 등)
└─ 📄 application.yml  # 환경 설정 (또는 application.properties)
...
- Spring Boot 기반 날씨 조회 및 일기 저장 서비스 구축  
...
이 프로젝트는 날씨 데이터를 기반으로 한 개인 맞춤형 일기 작성 시스템을 구현하며,  
실제 서비스 환경에서 확장 가능한 구조로 설계 및 개발되었습니다.
